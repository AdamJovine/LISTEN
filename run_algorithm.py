...
def parse_args() -> argparse.Namespace:
    ...
    ap.add_argument(
        ...
        help=(
            ...
            "follows the ordered contextual sections."
        ),
    )
    ap.add_argument(
        "--output-root",
...
def _render_base_prompt_text(
    ...
    if section_order:
        ...
        rendered = template.replace("{scenario_header}", "\n\n".join(ordered_parts))
...
def _build_filename_settings(config: Dict[str, Any]) -> Dict[str, Any] | None:
    """Return prompt/order settings that must distinguish output filenames."""
    settings: Dict[str, Any] = {}

    comparison_variant = config.get("comparison_prompt_variant_override")
    if comparison_variant is not None:
        settings["cmp"] = str(comparison_variant)

    utility_variant = config.get("utility_prompt_variant_override")
    if utility_variant is not None:
        settings["util"] = str(utility_variant)

    section_order = config.get("section_order")
    if section_order:
        settings["order"] = "-".join(str(part) for part in section_order)
...
def main():
    ...
    if args.dry_run:
        ...
        output_file = build_output_filename(
            ...
            comparison_settings=_build_filename_settings(config),
