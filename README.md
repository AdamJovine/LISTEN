...
submission. The repository tracks the curated plot and CSV artifacts in
`outputs/plots/`; re-running the scripts may produce additional local plot
...
(~620 MB compressed, ~7.3 GB and 11,815 JSONs uncompressed). The
raw JSONs are intentionally not committed to git; the curated plots
and CSV tables live under `outputs/plots/`. To skip Stage 1 and
regenerate plots from the released outputs:
...
The release also includes `SHA256SUMS.txt` and `MANIFEST.txt`.

| Asset                          | Size  | Scenario                 |
|--------------------------------|-------|--------------------------|
| `exam.tar.gz`                  | 144 M | `exam/`                  |
| `flights_chi_nyc.tar.gz`       |  78 M | `flights_chi_nyc/`       |
| `flights_ithaca_reston.tar.gz` |  61 M | `flights_ithaca_reston/` |
| `headphones.tar.gz`            | 336 M | `headphones/`            |
